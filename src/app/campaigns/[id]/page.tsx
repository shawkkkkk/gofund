...
type SettlementRow = { id:string; receipt_number:string; amount_cents:string; source_asset:"SOL"|"USDC"; source_amount_base_units:string; status:string; receipt_url:string|null; donation_reference:string|null; note:string|null };
...
export default async function CampaignPage({ params }:{ params:Promise<{id:string}> }) {
  ...
  const [campaignResult, tokenResult, assetResult, settlementResult] = await Promise.all([
    ...
    query<AssetRow>(
      ...
      " from settlements where campaign_id=$1 and status in ('QUEUED','PROCESSING','HELD','COMPLETED')" +
    ...
    query<SettlementRow>(
      "select id::text,receipt_number::text,amount_cents::text,source_asset,source_amount_base_units::text,status,receipt_url,donation_reference,note" +
      ...
      <div className="grid">
        {(["SOL","USDC"] as const).map((asset) => {
          ...
          return <article className="card" key={asset}>
            ...
            <p className="muted">indexed creator fees not yet reserved for settlement</p>
          ...
          <tbody>
            {settlementResult.rows.length ? settlementResult.rows.map((s) =>
              <tr key={s.id}>
                ...
                <td>{s.status}{s.status === "HELD" && s.note ? <><br/><small className="muted">{s.note}</small></> : null}</td>
